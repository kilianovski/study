...
class ArenaUnpickler(pickle.Unpickler):
    def find_class(self, module, name):
        print(module)
        # Replace 'old_module' with 'new_module'
        if module == "instructions.chatbot":
            module = "baseline_chatbot"
        return super().find_class(module, name)

def load_arena_pickle(path):
# Load the pickle file
    with open(path, "rb") as file:
        custom_unpickler = ArenaUnpickler(file)
        obj = custom_unpickler.load()
        return obj
...
def create_context(
    ...
    q_embeddings = llm_client.embeddings.create(input=question, model=engine).data[0].embedding

    # Get the distances from the embeddings
    # embedding_distances = distances_from_embeddings(q_embeddings, my_embeddings.embeddings_tensor, distance_metric='cosine')
    embedding_distances = 1 - F.cosine_similarity(t.tensor(q_embeddings)[None], my_embeddings.embeddings_tensor)

    if debug: print(f"""## Useful info
